...
def get_builds(s):
    ...
    if response.status_code == 200:
        ...
        if selected_script_element:
            ...
            if match:
                all_builds=match.group(1)
                # print(all_builds)
                try:
                    all_builds_parsed=json.loads(all_builds)
                    current_date=get_current_date()
                    build_list = []
                    for ele in all_builds_parsed:
...
def cluster_deploy_status(spy_link):
    ...
    #next implemlent functions to check for cluster creartion errors and e2e errors and call respective function to analyze the errors
...
def temporary_main_function():
    url_set = ['']    
    for url in url_set:
        build_list = get_builds(url) 
        for build in build_list:
            cluster_status=cluster_deploy_status(build["SpyglassLink"])
            print(build,cluster_status)
